$(document).ready(function (){
		  $("ul.main-nav li a").on('click',function(){
				...
				var header = $('div.header').height();
				 $('html, body').animate({
					  scrollTop: $("div."+classT).offset().top-parseInt(header)
				}, 1000);               
			});
		  ...
		  $("a.logo").on('click',function(){
				var header = $('div.header').height();					  
				 $('html, body').animate({
					  scrollTop: $("div#jssor_1").offset().top-parseInt(header)
				}, 1000);               
			});
	...
	$('a.menu-icon').click(function(){
		if($(this).hasClass('active')){
			...
			$('a.enq_btn').click(function(){
				if(validateBookNow()){
					var aUrl = "mail.php?xAction=contact_form&"+$('form#contact_form').serialize();
					$.ajax({
						type: 'post',
						url: aUrl,
